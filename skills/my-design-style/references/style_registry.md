...
This registry is the concrete-style lookup table for `my-design-style`. Use it to resolve style names, aliases, domain cues, priorities, and stable asset-policy handles. Detailed behavior belongs in each concrete style file; asset-folder file inventories remain opaque runtime data, but every concrete style must declare and maintain `assets/<style_name>/` plus `ASSET_MANIFEST.md` even when the inventory is empty.
...
- Hybrid work should declare one primary style and any secondary influence so the result stays intentional instead of averaged.
- Use assets through the active concrete implementation's policy; when another style's asset family is desired, treat that as an explicit hybrid or asset-source request.
