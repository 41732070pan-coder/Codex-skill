...
This is a color-and-layout style. Use interpreted color, linework, watermark-like depth, and denomination logic freely.
...
- Dense linework, guilloche-inspired curves, watermark-like panels, and premium editorial moments are welcome when they support hierarchy.
...
- Use any imagery, motif, or banknote-derived element that helps the design; real RMB photography, denomination strips, and currency motifs are fair game.
...
Use a deck-level visual rhythm plan so the RMB-inspired language feels valuable and layered without becoming a repeated paper template.
...
- `visualAnchorRule`: every slide or major section needs one non-body-text anchor: metric tile group, denomination color strip, chart, statement table, fine-line block, corner tick, process row, product/object image, currency motif, or controlled paper texture.
...
- `motifRotation`: rotate fine rules, corner ticks, guilloche-inspired curves, denomination strips, currency motifs, texture-off states, and table/charts.
- `assetFallbackRule`: when the visible inventory is thin, download task-relevant financial imagery, generate vectors, or use data visualization, tabular numerals, denomination swatches, paper/ink contrast, and runtime asset roles rather than red-gold filler.
- `variationCheck`: verify that each page has a value/data/culture anchor and color emphasis varies by role.
...
- Use paper/ink surfaces, fine rules, corner ticks, denomination strips, and banknote-derived motifs as the design calls for.
...
Use RMB-inspired ornament roles exposed by the runtime `AssetPolicy`, plus network-sourced and generated assets. The asset boundary may be edited by users; this style file does not enumerate or verify its files.

- `assetRoot`: `assets/renminbi_color_style/`.
- `importMode`: `style-owned`.
- `manifestFile`: asset-bundle handle.
- `availableAssets`: optional denomination strips, report corner frames, fine-line depth, watermark-like card panels, and financial ornament roles when exposed by the runtime policy.
- `usageRoles`: denomination color strips, report corner frames, fine-line depth, watermark-like card panels, generated in-artifact linework, icons, user-provided imagery, and network-sourced Pixabay/Iconfont assets.
- `placementRules`: keep decorative linework outside reading paths and away from data labels; use at most one major financial ornament per slide or screen.
- `fallbackPolicy`: when the visible inventory is thin, download task-relevant financial imagery, generate vectors, or use denomination color progressions, tables, chart systems, and coded fine rules.

For RMB-inspired work:

- Build value through color progression, fine rules, tables, chart systems, layered layout rhythm, and available assets.
- Use code-native color strips, generated corner rules, low-opacity linework, denomination strips, and banknote-derived motifs for hierarchy and palette rhythm.
- Combine modern financial UI structure with currency imagery as the design calls for.

## Surface Texture Policy

RMB-inspired texture support is enabled through the `transparent_textures` provider handle as abstract paper tactility.
...
- `allowedTokens`: ["paper-fibers", "textured-paper", "clean-gray-paper", "natural-paper", "light-paper-fibers", "beige-paper", "gold-scale", "brushed-alum", "pinstripe-light", "stardust", "low-contrast-linen"]
- `opacityRange`: [0.02, 0.05]
- `allowedSurfaces`: ["broad-background", "card-panel", "edge-band", "cover-wash"]
- `protectedSurfaces`: ["dense text", "chart labels"]
- `fallbackPolicy`: disable texture and keep the style through denomination color progressions, paper/ink contrast, fine rules, financial grids, tables, and code-native geometry.

Texture is a generic tactile substrate.

## Asset Rules

Use generated vectors, coded ornaments, runtime assets, user-provided files, network-sourced Pixabay/Iconfont assets, or banknote imagery as the design calls for.

Allowed:

- Generated or code-native financial ornaments such as color strips, corner rules, fine-line depth, and watermark-like panels.
- Fine-line patterns inspired by engraving or guilloche, created from curves, repeated rules, or low-opacity CSS/SVG.
- Watermark-like circles, floral silhouettes, landscape contours, architectural outlines, denomination strips, and currency motifs.
- Paper grain, fiber speckles, and subtle print misregistration when they do not reduce legibility.
- Iconography for wallet, invoice, ledger, chart, settlement, vault, auction, collection, and exchange.
- Real RMB photography and banknote-derived landmarks when they fit the subject.

## Modifier Compatibility

RMB modifiers can push the style toward fintech-blue, premium-auction, compliance-dashboard, ceremonial-report, or cultural-collection variants while preserving banknote-inspired color and depth.

- `acceptsModifiers`: true.
- `allowedTargets`: palette, motif, texture, layout, mood, asset.
- `allowedSources`: style-owned when declared, user-provided, generated-vector, code-native, network, shared-provider when enabled by the surface policy, and none.
- `defaultIntensity`: subtle for product/UI work, balanced for editorial or PPT section moments.
- `conflictPolicy`: compose modifiers around readability and denomination-family logic; keep the selected palette recipe coherent.
...
- Generated-vector linework, guilloche-inspired curves, watermark-like depth, paper-like surfaces, denomination strips, and currency motifs.
- Layout-density adjustments for reports or dashboards when information hierarchy remains orderly.
- User-provided or network-sourced imagery when it supports the content.

Modifier self-check additions:

- The output reads as banknote-inspired design.
- Added colors support the chosen denomination or documented palette recipe.

## Preview Option Sets

`getPreviewOptions(request, composedPlan)` exposes preview choices. In `previewMode: auto`, explicit preview is used only when ambiguity, stakes, or user request requires it. Otherwise the model creates an internal `StyleLock` from the default option sets and proceeds. When a preview is used, it should show the selected denomination palette, paper/ink contrast, fine-line components, a metric card, a table or chart fragment, and any texture choice. `applyStyleLock(styleLock, composedPlan)` must preserve the locked denomination family, texture, and linework strength in the final artifact.
...
- Other denomination options map one-to-one to the existing series cards and should be offered as replacements when the user wants a calmer, more analytical, or more premium mood.
- `texture-off` keeps RMB style through denomination color progression, fine rules, financial grids, and geometry.
- Do not use `linework-off` or shape-only delivery as the global default for ordinary non-wireframe visual artifacts. Use them only for explicit user constraints or dense data/readability needs. When the visible inventory is thin, download or generate assets.
- Style locks record the selected denomination series, texture token or `texture-off`, linework strength, and selected asset emphasis.
...
- `issues`: list any concrete RMB style, readability, layout, or data problems.
- `requiredFixes`: list required revisions before delivery.

Minimum checks:

- The design is banknote-inspired.
...
- The design feels trustworthy, valuable, and modern.
