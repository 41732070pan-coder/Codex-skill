...
| `skill-tune` | `skills/skill-tune/` | workflow | experimental | Tunes an existing skill after a concrete scenario run: isolated judge on output, improve until rubric excellence, log self-iter and tune_sessions artifacts. | Skill tune, self-iter loop, forward-test, rubric gate, isolated judge, scenario run with deliverable out. | Askill edits, self-iter.md, tune_sessions round artifacts, improve records. | Skill creation, rename, registry, governance audit belong to meta-skill; one-off domain runs without changing the skill. | Uses tune_session.py; per-target optional references/tune_rubric.md. |
