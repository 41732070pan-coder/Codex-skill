...
| --- | --- | --- | --- |
| `st-html-main` | Main concept | explicit | source-derived |

## 学习路线
| block title | 角色 | 深度 | 预计分钟 | 说明 |
| --- | --- | --- | --- | --- |
| HTML Noise Filtering | core | standard | 8 | 学习可追踪的正文机制。 |

## 核心讲解
HTML 的 `<main>` 区域保留正文；导航、侧栏和 cookie banner 作为 filler 跳过。 核心来源为 `st-html-main`。

## 跳过与延伸阅读
- 无需延伸阅读；低价值噪声已经跳过。

## 微测
1. **预测**：如果保留核心内容并过滤噪声，下一步会发生什么？
2. **用自己的话**：解释核心机制。
3. **应用**：给出一个可检查的小例子。

参考答案：应指出核心内容、噪声边界和可验证结果。

## 练习任务
用一个最小示例应用本节机制，并检查输出是否只包含需要学习的内容。
